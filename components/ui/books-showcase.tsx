...
export function BooksShowcase({
  ...
  const selectedHref = selectedCfg?.href ?? selectedCfg?.coverURL ?? selectedCfg?.images?.front ?? '#';
  const selectedDownloadHref = selectedCfg?.downloadUrl ?? selectedHref;
      ...
      {showDetailPanel && (
        ...
        >
          ...
          <div className={`mt-[34px] flex items-center gap-[18px] @max-[760px]:mt-[18px] ${dpChild(210)}`}>
            ...
            <div className="text-[19px] italic text-[#98a4d6]">E-book</div>
          ...
          >
            ...
            <a href={selectedDownloadHref} target="_blank" rel="noreferrer" className="inline-flex h-[54px] items-center gap-[10px] rounded-full bg-[var(--bs-peri)] px-[26px] text-[16.5px] font-semibold text-[#10152c] transition-[transform,filter] duration-[220ms] ease-[cubic-bezier(0.34,1.56,0.64,1)] hover:scale-[1.04] hover:brightness-105 @max-[760px]:h-12 @max-[760px]:px-5 @max-[760px]:text-[15px]">
              ดาวน์โหลด
            </a>
          </div>
        </div>
