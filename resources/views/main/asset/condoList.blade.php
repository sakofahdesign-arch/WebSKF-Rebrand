...
<div class="bg-gray-50 min-h-screen text-gray-800 font-sans" data-theme="light" x-data="{ loaded: false }" x-init="() => { setTimeout(() => loaded = true, 50) }">
    ...
    <div class="relative bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white py-20 shadow-lg overflow-hidden">
        ...
        <div class="absolute bottom-0 left-0 w-96 h-96 bg-fuchsia-300 rounded-full mix-blend-overlay filter blur-3xl opacity-20 -ml-20 -mb-20"></div>
        ...
        <div class="container mx-auto px-4 text-center relative z-10 transition-all duration-700 ease-out" 
            ...
            <h1 class="text-4xl md:text-5xl font-extrabold mb-4 drop-shadow-md">คอนโดมิเนียม</h1>
            <p class="text-lg md:text-xl font-light max-w-2xl mx-auto opacity-95">
                ตอบโจทย์ไลฟ์สไตล์คนเมือง สะดวกสบาย เดินทางง่าย พร้อมสิ่งอำนวยความสะดวกครบครัน
            </p>
        </div>
    </div>
        ...
        @if ($asset->isEmpty())
            <div class="card bg-base-100 shadow-xl text-center p-12 border border-base-200 transition-all duration-700 delay-100"
                ...
                <div class="card-body items-center text-center">
                    <h2 class="card-title text-2xl text-gray-600">ไม่พบรายการคอนโด</h2>
                    <p class="text-gray-500">ขณะนี้ยังไม่มีรายการคอนโดมิเนียมประกาศขาย</p>
        ...
        @else
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
                @foreach ($asset as $index => $item)
                    <a href="condo/{{$item->id}}" class="group h-full">
                        <div class="card bg-base-100 shadow-lg hover:shadow-2xl transition-all duration-300 border border-gray-100 h-full overflow-hidden opacity-0 translate-y-10"
                            ...
                            <figure class="relative h-56 overflow-hidden">
                                ...
                                <div class="absolute bottom-3 right-3 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-2 group-hover:translate-y-0">
                                    <span class="btn btn-sm btn-primary border-none shadow-lg bg-violet-600 hover:bg-violet-700">ดูรายละเอียด</span>
                                </div>
                            </figure>

                            <div class="card-body p-5">
                                <div class="flex items-center gap-2 text-xs text-gray-400 mb-2">
                                    <i class="far fa-calendar-alt text-violet-500"></i>
                                    {{ thaidate('j F Y', $item->date) }}
                                </div>

                                <h3 class="card-title text-lg font-bold text-gray-800 leading-snug group-hover:text-violet-600 transition-colors line-clamp-1" title="{{ $item->title }}">
                                    {{ $item->title }}
                                </h3>
                                ...
                                <div class="card-actions justify-start items-center border-t border-gray-100 pt-3 mt-auto">
                                    <div class="flex gap-3 text-xs text-gray-400">
                                        <span class="flex items-center gap-1"><i class="fas fa-key text-violet-500"></i> พร้อมเข้าอยู่</span>
                                        <span class="flex items-center gap-1"><i class="fas fa-parking text-violet-500"></i> มีที่จอดรถ</span>
                                    </div>
                                </div>
                            </div>
                        </div>
