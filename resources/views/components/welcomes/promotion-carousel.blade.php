@php
    $promotions = [
        [
            'id' => 1,
            'slide_image' => 'images/banners/bannerramadon.jpg',
            'modal_type' => null,
            'modal_content' => null,
        ],
        [
            'id' => 2,
            'slide_image' => 'images/banners/banner.jpg',
            'modal_type' => 'image',
            'modal_content' => 'images/banners/bannerdetail.jpg',
        ],
        ['id' => 3, 'slide_image' => 'images/banners/521-2.jpg', 'modal_type' => null, 'modal_content' => null], // No modal
        ['id' => 4, 'slide_image' => 'images/banners/515-1.jpg', 'modal_type' => null, 'modal_content' => null], // No modal
        [
            'id' => 5,
...
<section class="py-16 bg-gray-50">
    ...
    }" x-init="autoplay()" class="relative w-full group">
            ...
            @foreach ($promotions as $promo)
                <div x-show="activeSlide === {{ $loop->iteration }}"
                    ...
                    <img src="{{ asset($promo['slide_image']) }}"
                        class="block w-full h-full object-contain {{ $promo['modal_content'] ? 'cursor-pointer' : '' }}"
                        alt="โปรโมชัน"
        ...
        <div class="absolute z-30 flex -translate-x-1/2 bottom-5 left-1/2 space-x-3">
            @foreach ($promotions as $promo)
                <button @click="activeSlide = {{ $loop->iteration }}"
                    :class="{
                        'bg-white': activeSlide === {{ $loop->iteration }},
...
<!-- All Modals (Generated from Loop) -->
    @foreach ($promotions as $promo)
        @if ($promo['modal_content'])
            <div id="promotion-modal-{{ $promo['id'] }}" tabindex="-1" aria-hidden="true"
                ...
                <div
                    class="relative p-4 w-full {{ $promo['modal_type'] == 'video' ? 'max-w-4xl' : 'max-w-3xl' }} max-h-full
